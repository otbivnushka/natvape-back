...
export function startBot(app: INestApplication, dumbBot: TelegramBot) {
  ...
  const token = configService.get<string>('ADMIN_BOT_TOKEN', '');
  if (!token) {
      ...
      if (price) {
        ...
        if (price.step === 'awaiting_order_id') {
          ...
          if (!Number.isInteger(orderId) || orderId <= 0) {
            await bot.sendMessage(
              chatId,
              'Некорректный номер заказа. Попробуйте ещё раз:',
            );
            return;
          }
          ...
          await bot.sendMessage(
            chatId,
            `Заказ #${orderId}. Введите новую фактическую цену:`,
          );
          return;
        }
        ...
        if (!rawPrice || isNaN(newPrice) || newPrice < 0) {
          await bot.sendMessage(
            chatId,
            'Некорректная цена. Попробуйте ещё раз:',
          );
          return;
        }
        try {
